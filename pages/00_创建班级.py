...
if len(class_list) == 0:
    ...
    if len(student_list) == 0:
        ...
        for _ in range(row_num):
            ...
            for c in cols:
                if idx < total:
                    stu_name = student_list[idx]
                    if stu_name == st.session_state["random_pick_stu"]:
                        c.button(stu_name, use_container_width=True, type="primary")
                    else:
                        c.button(stu_name, use_container_width=True)
                    idx += 1
        ...
        with col1:
            if st.button("随机抽取1名学生", type="primary"):
                import random

                picked = random.choice(student_list)
                st.session_state["random_pick_stu"] = picked
                st.rerun()
        with col2:
            if st.button("清空抽取结果"):
                st.session_state["random_pick_stu"] = ""
        ...
        if st.session_state["random_pick_stu"] != "":
            st.markdown(
                f"""
                <div style="background:#165DFF;color:#fff;padding:20px;border-radius:10px;text-align:center;margin-top:10px;">
                    <h2>🎉 本次抽到学生：{st.session_state["random_pick_stu"]}</h2>
                </div>
                """,
...
if not select_class:
    ...
    with col_btn1:
        start_analysis = st.button("加载本班统计分析", type="primary")
    with col_btn2:
        load_detail = st.button("加载本班学生成绩明细", type="secondary")
    ...
    # 加载完整成绩明细（已修复变量名load_detail）
    if load_detail:
        st.divider()
        st.subheader("📋 本班该试卷全部学生成绩明细")
                    ...
                    if ans_col in class_score.columns:
                        ...
                        for ans in valid_ans:
                            if ans in abcd_count:
                                abcd_count[ans] += 1
                    bar_df = pd.DataFrame({"选项": list(abcd_count.keys()), "选择人数": list(abcd_count.values())})
                    st.bar_chart(bar_df, x="选项", y="选择人数", height=240, use_container_width=True)
                else:
                    bar_df = pd.DataFrame({"答题情况": ["答对人数", "答错人数"], "人数": [right_num, wrong_num]})
